...
##### Stage 3. Data cleansing and re-categorisation
##### Stage 4. Tidying up the data on economic impact and adjusting for inflation
##### Stage 5.  Handling economic impact and health hazards separately
##### Stage 6. Measuring overall harm to population health
##### Stage 7. Further simplification of categories
##### Stage 8. Summarizing and grouping the data in preparation for plotting
...
* it is faster
* it has granular configuration options for each column, such as date formats
* it does not try to load all strings as factors
...
```{r data_quality_problems, fig.width=8}
...
```{r converting_value_and_exp_to_new_value}
...
```{r get_inflation_rates_and_adjust_damages_costs}
library(XML)
...
```{r further_categorisation_of_events}
...
```{r determining_event_and_quarter_totals_for_harm}
library(data.table)
...
```{r determining_event_and_quarter_totals_for_damage}
...
* The wish to plot a mix of individual and summary data points on the same panel (recorded fatalities and overall harm by type and quarter)
* Speed of handling the large data set for economic impact (compared with sapply/mapply)
* The ease of use
* A learning recap for this researcher

## Results

#### Overall harm by event type by calendar Quarters

```{r plotting_harm_results, fig.height=7, fig.width=8}
library(lattice)

# common parameters
clrs <- c("#F248BA", "#886633", "#4488AA")
dot <- c(15, 20, 4)
alph = c(0.2, 1, 1, 1)
dotsize = c(1.5, 1.1, 1)
...
print(harmByY, pos = c(0.8, 0.0490, 1, 0.991))
```
...
```{r plotting_damage_results, fig.height=7, fig.width=8}
library(ggplot2)
...
```{r comparison_property_crop_damage_scales}
options(digits = 9)
